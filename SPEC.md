...
`next/link` 위에 hover/focus 시 데이터 prefetch를 얹은 컴포넌트. `next/link`의 chunk prefetch와 데이터 prefetch를 통합. 무엇을 warm할지 두 가지 방법:

- `prefetch={[itemsQuery()]}` — warm할 query를 명시적으로 나열. decoupled이지만 loader의 실제 query set과 drift할 수 있다.
- `loader={targetLoader}` — 목적지 페이지의 loader를 hover 시 실행(`validate` 적용, `beforeLoad`는 실행 안 함, throw는 swallow). loader가 single source of truth이므로 drift가 없다. 단 loader를 참조하면 eager import되므로(lazy chunk 분리 없음), same-page param 링크나 coupling이 허용될 때 사용.

```tsx
<PrefetchLink href="/items" prefetch={[itemsQuery()]}>Decoupled</PrefetchLink>
...
4. **`next/link` prefetch와의 통합 한계.**
   `next/link`의 prefetch는 chunk만 가져온다. 데이터 prefetch는 `<PrefetchLink>`를 써야 한다. 사용자가 일반 `<Link>`를 쓰면 hover 시 chunk만, 클릭 시 loader가 데이터 fetch — 즉 두 번의 latency가 직렬화될 수 있다. `<PrefetchLink loader={...}>`는 drift를 없애지만 loader를 eager import하므로, cross-page 링크에서는 lazy chunk 분리를 위해 `prefetch` 리스트가 여전히 필요하다.
